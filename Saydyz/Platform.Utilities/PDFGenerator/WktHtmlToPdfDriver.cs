...
{
    public abstract class WktHtmlToPdfDriver
    {
        /// <summary>
        ...
        public static byte[] Convert(string wkhtmlPath, string switches, string html)
        {
            ...
            {
                exeLocation = Path.Combine(wkhtmlPath, "Windows", "wkhtmltopdf.exe");
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                exeLocation = Path.Combine(wkhtmlPath, "Mac", "wkhtmltopdf");
            }
            else
            {
                exeLocation = Path.Combine(wkhtmlPath, "Linux", "wkhtmltopdf");
            }

            if (!File.Exists(exeLocation))
            {
                throw new Exception("wkhtmltopdf not found, searched for " + exeLocation);
            }
